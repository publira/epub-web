...
Everything in the repository is **English** apart from the translated UI copy and the test data that exercises non-ASCII text: `README.md`, `CONTRIBUTING.md`, code comments, test labels (`describe` / `it`, `t.Run`), commit messages, Issues, and pull requests. The UI catalogs are `locales/*.json`, embedded by the Go server and imported by the SPA through the `@publira/epub-web-locales` workspace package: `en.json` defines the message IDs, every file carries the same keys, and a new language also needs its entry in `locales/index.ts`. `EPUB_WEB_SUPPORTED_LANGUAGES` selects EPUB metadata languages, not the interface language.
...
Middleware is a `with*` wrapper composed in `main.go`. Upload routes get `withFetchSiteCheck(withTimeout(withLimit(...)))` — give a new upload endpoint the same chain. An HTML page is served through `handleLocalizedPage` and is written with the `en.json` title, description, and `og:locale`, which the server rewrites into the request's language; its `<!-- epub-web:head -->` marker receives the head elements that depend on the request, such as the canonical link and the `hreflang` alternates.
