...
export const Dialog = ({
  ...
  return (
    <dialog
      className={twMerge(
        "fixed top-1/2 left-1/2 m-0 max-h-[80dvh] w-[min(680px,calc(100%-2rem))] -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-surface border border-border bg-card p-0 text-card-foreground shadow-floating backdrop:bg-foreground/20 backdrop:backdrop-blur-xs",
        className
      )}
      ref={dialogRef}
      {...props}
...
export const DialogClose = ({ className, ...props }: DialogCloseProps) => (
  <form
    ...
    method="dialog"
  >
    <Button size="icon" type="submit" variant="ghost" {...props}>
